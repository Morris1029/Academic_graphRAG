...
@tool
def read_node_content(node_id: str):
    """
    【重要】获取与该节点关联的原始文本块(Chunk)内容。
    当图关系(get_neighbors)无法提供足够信息时，务必使用此工具读取原文！
    """
    retriever = get_retriever()
    content = retriever.get_node_content(node_id)
    if not content:
        return "No content found for this node."
    return f"Chunk Content for {node_id}:\n{content[:2000]}..." # 限制长度防止Context溢出
...
def execute_python_analysis(code_snippet: str):
    ...
    # 注入工具函数供代码调用
    tools_map = {
        "search_entity": lambda q: search_entity.invoke(q),
        "get_neighbors": lambda n: get_neighbors.invoke(n),
        "read_node_content": lambda n: read_node_content.invoke(n),
        "print": print,
        "sorted": sorted,
        "len": len,
        "set": set,
    ...
    try:
        ...
        if result_val and str(result_val) != str(stdout_val).strip():
            final_output.append(f"Result Variable:\n{result_val}")

        if not final_output:
            return "Code executed successfully but produced no output. Did you forget to print() or set 'result'?"
...
# --- 3. 导出工具列表 ---
def get_all_tools():
    return [search_entity, get_neighbors, read_node_content, execute_python_analysis]