...
class KTRetriever:
    ...
    def search_nodes_only(self, query: str, top_k: int = 5) -> List[Dict]:
        """
        只进行实体检索，返回结构化的节点信息列表，不包含三元组或Chunk。
        供 Agent 确认实体是否存在或获取实体属性。
        """
        query_embed = self._get_query_embedding(query)
        # 复用你原有的 _node_relation_retrieval 中的一部分逻辑
        # 这里只做向量搜索，不进行多跳扩展
        raw_results = self.faiss_retriever.node_index.search(...)

        results = []
        for node_id in raw_results:
            node_data = self.graph.nodes[node_id]
            results.append({
                "id": node_id,
                "name": node_data.get("name"),
                "description": node_data.get("description"),
                "attributes": node_data.get("properties")  # 关键：返回属性供代码分析
            })
        return results
